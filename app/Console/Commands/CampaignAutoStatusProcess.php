...
class CampaignAutoStatusProcess extends Command
{
    ...
    public function processAutoOnOpenNextDay()
    {
        $this->info("Process campaign auto On OpenNextDay");
        $campaigns = Campaign::query()
            ->where('status', '=', 0)
            ->where('open_next_day', '=', 1)
            ->get();

        if ($campaigns->count() === 0) {
            $this->warn("No campaign to auto on next day");
        }
        ...
        foreach ($campaigns as $campaign) {
            ...
            try {
                $tag = '[AUTO ON NEXT DAY] [' . $campaign->id . ']';
                $this->info("$tag Processing campaign {$campaign->id} {$campaign->name}");
                ...
                $log->campaign_id = $campaign->id;
                $log->time = date('Y-m-d H:i:s');
                $log->title = "[OppenNextDay] Tự động bật campaign: " . $campaign->id . " | " . $campaign->name . ' lúc '. $log->time;
                $log->content = $log->title;
                $this->warn( $tag . $log->title);
                $log->save();
            } catch (\Throwable $ex) {
