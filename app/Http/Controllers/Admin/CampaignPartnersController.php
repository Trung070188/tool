    ...
    public function data(Request $req)
    {
        ...
        if ($req->keyword) {
            $query->where('name', 'LIKE', '%' . $req->keyword . '%')
                ...
                ->orwhereHas('campaign', function ($q) use ($req)
                {
                    $q->where('name','LIKE', '%'.$req->keyword . '%')
                        ->orwhere('id', $req->keyword);
                })
                ->orWhereHas('partner', function ($q) use ($req)
                {
                   $q->where('name', 'LIKE', '%'.$req->keyword . '%');
                });
        }
...
//        $query->createdIn($req->created);
    ...
    public function openNextDay(Request $req)
    {
        ...
        $entry->open_next_day = $req->open_next_day ? 1 : 0;
        $entry->save();
    ...
    public function shareData(Request $req)
    {
        $id = $req->get('id');
        $entry = CampaignPartner::find($id);
        ...
        $entry->share_data = $req->share_data ? 1 : 0;
        $entry->save();
