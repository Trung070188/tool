...
class CampaignsController extends AdminBaseController
{
    ...
    public function save(Request $req)
    {
        ...
        if (isset($data['id'])) {
            ...
            $event = new EventLog();
            $event->campaign_id = $entry->id;
            $event->title = "Thay đổi campaign";
            ...
            $oldData['icon'] = $oldData['icon'][0]['url'];
            if(@$changes['icon'])
            {
                $changes['icon'] = json_decode($changes['icon'])[0]->url;

            }
            foreach ($changes as $k => $newValue) {
                if (isset($ignored[$k])) {
                    continue;
                }

                $oldValue = $oldData[$k] ?? '';
                $contents[] = "$k: $oldValue --> $newValue";
            }
            $event->content = "Thay đổi: <br>" . implode("<br>", $contents);
            $event->save();
            DB::commit();
            ...
            $entry->fill($data);
            $event = new EventLog();
            $event->content = "Tạo campaign mới";
            $event->title = "Tạo campaign mới";
            $event->time = date('Y-m-d H:i:s');
            DB::beginTransaction();
            $event->save();
            $entry->save();
    ...
    public function switchFake(Request $req)
    {
        $id = $req->entry['id'];
        Campaign::where('id', $id)->update(['is_fake_on' => $req->entry['is_fake_on']]);
        return [
            'code' => 200,
            'message' => 'Đã lưu'
        ];
...
//            ]
    ...
    public function dataDetail(Request $req)
    {
        ...
        $results = DB::table('campaign_installs')->where('campaign_id', $req->id)
            ->selectRaw('DATE(installed_at) AS date, COUNT(campaign_id) AS campaign_count,COUNT(faked_at) AS count_fake')
        ...
        foreach ($results as $result) {
            $install_counts[$result->date] =[
                'campaign_count' => $result->campaign_count,
                'count_fake' => $result->count_fake
        ...
        for ($i = 0; $i < $num_days; $i++) {
            ...
            $campaign_count = isset($install_counts[$date]['campaign_count']) ? $install_counts[$date]['campaign_count'] : 0;
            $fake_count = isset($install_counts[$date]['count_fake']) ? $install_counts[$date]['count_fake'] : 0;
            $data[] = [
