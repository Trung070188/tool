                            ...
                            <div class="table-responsive">
                                <table class="table mg-b-0 text-md-nowrap">
                                    ...
                                    <tr v-for="entry in entries">
                                        <td>{{entry.date}}</td>
                                        <td>{{entry.campaign_count}}</td>
                                        <td>{{entry.campaign_count - entry.fake_count}}</td>
                                        <td>{{entry.fake_count}}</td>
                                    </tr>
                                    <tr>
                                        <td>Tổng</td>
                                        <td>{{totalInstall}}</td>
    ...
    methods: {
        ...
        for (let i=0; i<A.length; i++) {
            if (A[i].date < 10) {
                ...
                result.push({date: z, campaign_count: A[i].campaign_count, fake_count: A[i].fake_count});
            } else {
                result.push({date: A[i].date, campaign_count: A[i].campaign_count, fake_count: A[i].fake_count});
            }
        }
        return result;
},
        async load() {
            ...
            this.totalInstall = res.data.reduce((accumulator, currentValue)=>{
                return accumulator + parseInt(currentValue['campaign_count']);
            },0);
            this.totalFake = res.data.reduce((accumulator, currentValue)=>{
                return accumulator + parseInt(currentValue['fake_count']);
            },0);
            // this.this.totalShare = res.data.reduce((accumulator, currentValue)=>{
            //     return accumulator + parseInt(currentValue['campaign_count']-currentValue['fake_count']);
            // },0);
